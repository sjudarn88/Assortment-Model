...
,items_purchased_by_store_members AS (   
    SELECT     b.locnnbr
          ...
          WHERE     b.TranDt >'2017-04-30'     
           ...
           HAVING     UNITS > 0 )
,items_to_be_introduced AS (   SELECT     a.storeA
                                          ,a.num_sim_mem
                                          ,a.soar_no
                                          ,a.div_no
                                          ,a.div_nm
                                    ...
                                    ON     a.storeA = b.locnnbr     
                                           AND a.prodirlnbr = b.prodirlnbr   
                                    WHERE     b.locnnbr IS NULL     
                                           AND b.prodirlnbr IS NULL   
                                    ORDER BY     storeA )
,sample AS (   SELECT     *   
                    FROM     items_to_be_introduced )
,inv AS (   SELECT     CAST(FLOOR(SKU_ID/1000) AS int64) AS prodirlnbr
                     ...
                     WHERE     WK_END_DT BETWEEN DATE('2017-01-01')     
                                AND DATE('2017-12-31')     
                                AND TTL_UN_QT > 0   
                     GROUP BY  1,2,3,4 )
,rcp AS (   SELECT     CAST(FLOOR(SKU_ID/1000) AS int64) AS prodirlnbr
                      ,locn_nbr
                     ...
                     FROM (     SELECT       DISTINCT SKU_ID AS sku_id
                                      ...
                                      WHERE       DAY_NBR BETWEEN '2017-01-01'       
                                                  AND '2017-12-31' )   
                     GROUP BY     1,2   
                     ORDER BY     2 )
,store_items AS (   SELECT     s.storeA
                              ,s.num_sim_mem
                              ,s.div_no
                              ,s.div_nm
                              ,s.ln_no
                         ...
                         LEFT JOIN (     SELECT       *     
                                              FROM       rcp     
                                              WHERE       prodirlnbr IN ( SELECT   prodirlnbr       
                                                                                  FROM   sample       
                                                                                  GROUP BY  1) ) AS e   
                         ON     s.prodirlnbr = e.prodirlnbr     
                                AND s.storeA = e.locn_nbr   
                         LEFT JOIN (  SELECT       prodirlnbr
                                                   ,locn_nbr
                                                   ,MAX(on_hand_inv_units) AS max_on_hand_inv_units     
                                              FROM       inv     GROUP BY  1,2) AS f   
                          ON     s.prodirlnbr = f.prodirlnbr     
                                  AND s.storeA = f.locn_nbr   
                          WHERE     e.ttl_reciept_units IS NULL     
                                  AND coalesce(f.max_on_hand_inv_units, 0) < 6 
,all_sales_L1Y AS (   SELECT     b.locnnbr
                                ,c.soar_no
                                ,c.div_no
                                ,c.div_nm
                                ,c.ln_no
