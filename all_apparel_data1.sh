...
,receipts as (
	...
	from
		(
		select 		distinct SKU_ID as sku_id
	...
	inner join
		(
		select		srsgrp_kmtbsnss_nbr as gro_nbr
				, srsvrtclbsnss_kmtdvsn_nbr as vbs_nbr
				, srsdvsn_kmtdept_nbr as div_nbr
				, srsline_kmtctgryclstr_nbr as ln_nbr
				, srssbline_kmtctgry_id as sbl_nbr
				, srsclss_kmtsbctgry_nbr as cls_nbr
				, srssku_kmtksn_id as sku_id
			from	`syw-analytics-repo-prod.l2_enterpriseanalytics.merchlvl90skuandksn`
...
, inventory_wk_prod_data as (
	Select
		...
		,CONCAT(CAST(a.div_nbr AS STRING), "_", CAST(a.ln_nbr AS STRING)) as Line_Product_Type 
	from
	(
   		select   div_nbr
			, ln_nbr
			...
			, wk_end_dt
			, on_hand_inv_units
			, damaged_or_unsellable_inv_units
			, in_transit_inv_units
			, on_order_inv_units
   		from `apparel_als_all_app.inventory`
   		--where div_nbr in (7, 41, 43)  
		-- (7, 31, 2, 16, 41, 43) 
		-- and --locn_nbr = 1125 and div_nbr = 7 
		-- and sku_id in (8214498010) 
		-- and div_nbr = 7
   	) a
	LEFT JOIN
	(
	...
	(
		select 
			...
			, Product_Type
			, IMA_Product_Type  
		from `syw-analytics-ff.apparel_als_all_app.basic_spin_attributes_apparel` ) c
	...
	(
  		SELECT
			...
			ON a.ProdIrlNbr=b.prd_irl_no
    			LEFT JOIN `syw-analytics-repo-prod.crm_mart1_tbls.crm_sears_retail_fty` C 
			ON a.OrignlFcltyNbr=C.fty_id_no
		...
		from (
					...
					, ITM_NO
                            	...
                            	where
                              		...
                              		wk_nbr > 201700 and wk_nbr < 201800 
					--probbly we need to do it by the season of the item, but this should work as well, need to check
		...
		FROM (
 			 ...
 			 FROM (
 			   	SELECT
 			     		...
 			     		Season_Correspondence,
 			     		assortment_id
 			   	FROM 	`apparel_als_all_app.stores_weekly_assortment_proc` 
 			  	WHERE	assortment_id IS NOT NULL  and Product_Type is not null) )
	...
	from (
		...
		from (
			select 	wk_nbr,
				...
				Class_Product_Type
			from 	`apparel_als_all_app.inventory_wk_prod_data`  
			where	PRD_IRL_NO is not null and
				wk_nbr > 201700 and wk_nbr < 201800 
				--probbly we need to do it by the season of the item, but this should work as well, need to check
				and on_hand_inv_units > 0 or SoldQty > 0 or ttl_reciept_units > 1
			) a
